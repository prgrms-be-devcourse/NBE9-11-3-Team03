...
class SecurityConfig(
            ...
            .exceptionHandling { exception ->
                exception
                    .authenticationEntryPoint { _, response, _ ->
                        writeErrorResponse(response, HttpServletResponse.SC_UNAUTHORIZED, "401", "로그인이 필요합니다.")
                    } // 로그인은 성공했지만 필요한 권한이 없는 API에 접근했을 때 실행된다.
                    .accessDeniedHandler { _, response, _ ->
                        writeErrorResponse(response, HttpServletResponse.SC_FORBIDDEN, "403", "접근 권한이 없습니다.")
                    }
            } // H2 콘솔을 iframe으로 열 수 있도록 개발 중에만 같은 출처 frame을 허용한다.
            .headers { it.frameOptions { frame -> frame.sameOrigin() } }
            .authorizeHttpRequests { auth ->
                auth // 회원가입, 로그인, 토큰 재발급은 access token 없이 접근할 수 있어야 함.
                    .requestMatchers(HttpMethod.POST, "/api/auth/signup", "/api/auth/login", "/api/auth/reissue")
                    .permitAll() // Swagger와 H2 콘솔은 개발 중 API 확인을 위해 열어둔다.
                    .requestMatchers(
                        "/swagger-ui/**",
                        "/swagger-ui.html",
                        "/v3/api-docs/**",
                        "/h2-console/**",
                        "/uploads/**"
                    ).permitAll() // 관리자 API는 ADMIN 권한이 있는 사용자나 개발용 ADMIN 토큰만 접근할 수 있다.
                    .requestMatchers("/api/admin/**").hasRole("ADMIN") // 비회원은 축제목록조회와 축제상세조회만 가능하도록 설정했다.
                    .requestMatchers(HttpMethod.GET, "/api/festivals", "/api/festivals/*")
                    .permitAll() // 위에서 허용한 API를 제외한 나머지 API는 JWT 또는 개발용 토큰 인증이 필요
    ...
    private fun writeErrorResponse(
        ...
        objectMapper.writeValue(response.writer, RsData<Void>(code, message, null))
    }
}
