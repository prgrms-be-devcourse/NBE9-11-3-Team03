...
public class ReviewLikeController {
    ...
    public ResponseEntity<ApiRes<ReviewLikeResponse>> likeReview(
            @PathVariable Long reviewId,
            Authentication authentication
    ) {
        String loginId = authentication.getName();

        ReviewLikeResponse response = reviewLikeService.likeReview(reviewId, loginId);
    ...
    public ResponseEntity<ApiRes<ReviewLikeResponse>> cancelLikeReview(
            @PathVariable Long reviewId,
            Authentication authentication
    ) {
        String loginId = authentication.getName();

        ReviewLikeResponse response = reviewLikeService.cancelLikeReview(reviewId, loginId);
