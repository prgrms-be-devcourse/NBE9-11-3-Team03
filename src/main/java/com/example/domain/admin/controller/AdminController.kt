...
class AdminController(
    ...
    fun getMemberList(
        ...
        return ResponseEntity.ok<RsData<MemberPageResponse>>(
            RsData<MemberPageResponse>(
                "200",
                "회원 목록 조회 성공",
                memberPage
            )
    ...
    fun getReportMemberList(
        ...
        val memberList = memberService.getReportMembers(pageable)
        return ResponseEntity.ok<RsData<MemberPageResponse>>(
            RsData<MemberPageResponse>(
                "200",
                "신고 누적회원 조회 성공",
    ...
    fun getReportReview(
        ...
        val reviewList = reviewService.getReportReview(pageable)
        return ResponseEntity.ok<RsData<AdminReviewReportPageResponse?>?>(
            RsData<AdminReviewReportPageResponse?>(
                "200",
    ...
    fun processReview(
        ...
        val adminLoginId = authentication.getName()
        log.info(
            "[ADMIN] 리뷰 상태 변경 요청 - adminLoginId={}, reviewId={}, action={}",
            adminLoginId,
            reviewId,
            req.action
        )
        val res = reviewService.processReviewAction(reviewId, req.action)
        val msg = if ("Blind".equals(req.action, ignoreCase = true))
            "리뷰가 블라인드 처리되었습니다."
        else
            "리뷰 신고횟수가 초기화 되었습니다."
        log.info(
            "[ADMIN] 리뷰 상태 변경 완료 - adminLoginId={}, reviewId={}, action={}, result={}",
            adminLoginId,
            reviewId,
            req.action,
            msg
        )
        return ResponseEntity.ok<RsData<AdminReviewBlindResponse?>?>(
            RsData<AdminReviewBlindResponse?>(
                "200",
                msg,
                res
            )
    ...
    fun memberWithdraw(
        ...
        return ResponseEntity.ok<RsData<AdminMemberWithdrawnResponse?>?>(
            RsData<AdminMemberWithdrawnResponse?>(
                "200",
                "회원이 강제 탈퇴 처리되었습니다.",
                res
            )
